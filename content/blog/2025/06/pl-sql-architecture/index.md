...
- Stored into the database with names
- Compiled Only Once
- Can be called by another block or application
- Safer than anonymous blocks because user needs the required privilege
- Can return values
- at face-value functions can return values, but procedures can not (we
  will see how procedures can later)
- Can take paremeters
- Functions and Procedures are called as subprogramsa

### Examples

These examples are of anonymous blocks, setting us up for procedures and
functions
...
Procedures can do similar manipulations but we occassionally need to work
with specific subsets of our data.  We can declare our procedures to accept
(IN) or return (OUT).

Between the parameter name and the type, we can pass the mode of the
paramter.  `IN` signals that we are going to be passing a value into a
procedure to do something, where as `OUT` signifies that the caller will
get the value of the specified paramter after the execution of the
procedure.  

If we declare `IN OUT` mode, the caller will both send a value as a
parameter and receive the value of that parameter after the procedure runs.  

`IN` is the default procedures.
...
### Named - Mixed Notations & Default Options For Procedures

We can use named notation `parameter_name => 'some_val'` or `parameter_name
=> v_some_var` up until we declare a positional argument.  a, 
